...
from InfiniteClone.utils.database import get_assistant
from config import API_ID, API_HASH
from InfiniteClone import app
from config import OWNER_ID
from InfiniteClone.misc import SUDOERS
from InfiniteClone.utils.database import get_assistant, clonebotdb
from InfiniteClone.utils.database.clonedb import has_user_cloned_any_bot
from config import LOGGER_ID, CLONE_LOGGER
import requests
from InfiniteClone.utils.decorators.language import language
import pyrogram.errors

from InfiniteClone.utils.database.clonedb import get_owner_id_from_db
from config import SUPPORT_CHAT, OWNER_ID

from datetime import datetime
CLONES = set()

C_BOT_DESC = "Wᴀɴᴛ ᴀ ʙᴏᴛ ʟɪᴋᴇ ᴛʜɪs? Cʟᴏɴᴇ ɪᴛ ɴᴏᴡ! ✅\n\nVɪsɪᴛ: @snowy_x_musicbot ᴛᴏ ɢᴇᴛ sᴛᴀʀᴛᴇᴅ!\n\n - Uᴘᴅᴀᴛᴇ: @dark_musictm\n - Sᴜᴘᴘᴏʀᴛ: @yukieee_03"
    ...
    if len(message.command) > 1:
        ...
        try:
            ai = Client(
                ...
                API_HASH,
                bot_token=bot_token,
                plugins=dict(root="InfiniteClone.cplugin"), 
            )
            await ai.start()
            bot = await ai.get_me()
        ...
        try:
            ...
            await mi.edit_text(
                f"<blockquote>⚠️ <b>ᴇʀʀᴏʀ:</b>\n\n<code>{e}</code>\n\n**ᴋɪɴᴅʟʏ ғᴏᴡᴀʀᴅ ᴛʜɪs ᴍᴇssᴀɢᴇ ᴛᴏ @cyber_github ᴛᴏ ɢᴇᴛ ᴀssɪsᴛᴀɴᴄᴇ</blockquoote>"
            )
    else:
